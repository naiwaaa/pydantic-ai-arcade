# Arcade.dev Integration for Pydantic AI

`pydantic-ai-arcade` is a lightweight integration layer that connects
[Arcade.dev](https://arcade.dev) tools with [Pydantic AI](https://ai.pydantic.dev),
enabling GenAI agents to securely interact with external services like Gmail, Slack,
Spotify using type-safe interfaces.

## Installation

```bash
pip install pydantic-ai-arcade
```

## Example Usage

See the [examples](./examples) for usage examples

## Resources

- [Arcade.dev Documentation](https://docs.arcade.dev/en/home)
- [Pydantic AI](https://ai.pydantic.dev/)
